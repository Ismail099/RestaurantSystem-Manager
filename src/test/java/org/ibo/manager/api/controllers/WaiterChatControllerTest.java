...
import org.ibo.manager.api.mapper.MessageMapper;
...
import org.ibo.manager.services.WaiterChatServiceImpl;
...
public class WaiterChatControllerTest {
    ...
    void testGetMessages() {
        ...
        mockMvc.perform( get( "/api/chat/messages" ).accept( MediaType.APPLICATION_JSON ) ).andExpect( status().isOk() )
                ...
                .andExpect( jsonPath( "$['messages'][0].seen", is( messageToWaiter.getSeen() ) ) )
                ...
                .andExpect( jsonPath( "$['messages'][1].seen", is( messageToManager.getSeen() ) ) );

        // Waiter has seen messages sent to him
        Database.getMessages().stream().filter( message -> message.getReceiver().equals( Util.CHAT_USER_WAITER ) ).forEach( message -> Assertions.assertTrue( message.getSeen() ) );
        Mockito.verify( waiterChatService, Mockito.times( 1 ) ).setAcknowledged();
    }
